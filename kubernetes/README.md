...
The shared K3s cluster hosts five namespace-isolated environments:

| Environment | Namespace | Ingress host |
| --- | --- | --- |
| Development | `bookstore-dev` | `dev.bookstore.example.com` |
| Test | `bookstore-test` | `test.bookstore.example.com` |
| Performance | `bookstore-perf` | `perf.bookstore.example.com` |
...
Development, test, and performance deployments use the shared cluster without
an environment-specific branch. Before staging or production deployment,
configure the matching GitHub Environment with required reviewers. Public DNS
names can replace the example hosts when a domain is available.
